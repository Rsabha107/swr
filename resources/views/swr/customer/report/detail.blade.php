        ...
        <div class="row align-items-center">
            ...
            <div class="col-auto">
                <div class="btn-list gap-2">
                    @if($report->canEdit())
                        <a href="{{ route('swr.report.edit', $report->id) }}" class="btn btn-primary">
                            <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 7h-1a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-1" /><path d="M20.385 6.585a2.1 2.1 0 0 0 -2.97 -2.97l-8.415 8.385v3h3l8.385 -8.415z" /><path d="M16 5l3 3" /></svg>
                            Edit
                        </a>
                    @endif
                    @if($report->documents->count() > 0)
                        <a href="{{ route('swr.report.gallery', $report->id) }}" class="btn btn-secondary">
                            ...
                            <div class="mb-3">
                                ...
                                <div>
                                    @foreach($report->innovationFunctionalAreas as $pivot)
                                        <span class="badge bg-info me-1 mb-1">{{ $pivot->functionalArea?->fa_code ?? $pivot->functionalArea?->name ?? 'N/A' }}</span>
                                    @endforeach
                                </div>
                            </div>
                        @endif
                            ...
                            <div class="mb-3">
                                ...
                                <div>
                                    @foreach($report->challengeFunctionalAreas as $pivot)
                                        <span class="badge bg-warning me-1 mb-1">{{ $pivot->functionalArea?->fa_code ?? $pivot->functionalArea?->name ?? 'N/A' }}</span>
                                    @endforeach
                                </div>
                            </div>
                        @endif
