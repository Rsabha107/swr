...
use App\Models\Swr\DayType;
use App\Models\Swr\Event;
use App\Models\Swr\Venue;
use App\Models\Swr\WorkforceDailyReport;
use App\Models\Swr\WorkforceDailyReportDocument;
...
class WorkforceDailyReportController extends Controller
{
    ...
    {
        $events = Event::all();
        return view('swr.customer.report.list', compact(
            'events'
        ));
    }
    ...
    public function gallery($id)
    {
        ...
        return view('swr.customer.report.gallery', compact(
            'events',
            'report',
        ));
    }
    ...
    public function list(Request $request)
    {
        ...
        $ops = $ops->paginate($limit)->through(function ($op) {
            ...
            $actions_pass =
                '<a href="' . route('swr.report.pdf', $op->id) . '" target="_blank" class="btn btn-sm me-1" data-table="report_table" data-id="' .
                $op->id .
            ...
            if ($op->photos()->exists()) {
                ...
                $href_route = route('swr.report.gallery', ['id' => $op->id]);
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('swr.report')->with($toastr_message);
    }
    ...
    public function save_pass_pdf($swr, $seq)
    {
        ...
        $pdf = Pdf::loadView('swr.admin.report.rpdf', $data);
